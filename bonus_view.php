...
if (isset($_GET['ids'])) {
   ...
   $bnsQry = mysqli_query($con,"SELECT a.id as ids,a.*,b.*,c.* FROM hr_bonus a, master_type_dtls b, mstr_emp c WHERE a.b_type=b.mstr_type_value AND a.created_by=c.id AND a.id='$ids' ORDER BY a.id DESC");
            ...
            <div class="row">
              ...
              <div class="col-lg-2 col-md-2 col-sm-1 col-xs-1">
                 <div class="list_icon text-right mt-1">
                     <a href="<?php if($viewid==1){ echo 'add_bonus_list.php';}else if($viewid==2){ echo 'manage_bonus_list.php';}?>"><i class="fa-solid fa-square-left fa-2xl"></i></a>
                   </div>
              </div>
           </div>
           <div class="row">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12"> 
                   ...
                   <form name="form" method="post" class="forms-sample">
                      <div class="form-row mt-1">
                         ...
                         <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                            <div class="form-group">
                               <label for="enter_by">Applicable Form :</label>
                               <?=$rows->applicable;?>
                            </div>
                         </div>
                         <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
...
$('#approve_Submit').on('click', function(event) {
   ...
   $.ajax({
       ...
       data: aprform.serialize() + "&action=approve&viewid="+viewid,
       type: 'POST',
         ...
         // return false;
           if (data == 1) {
               swal({
                  ...
                  if (viewid==1) {
                     window.location = "add_bonus_list.php";
                  }else if(viewid==2){
                     window.location = "manage_bonus_list.php";
                  }
               });
           }
       }
...
$('#recheck_Submit').on('click', function(event) {
   ...
   $.ajax({
       url: "php_function/ajax_bonus_status_updt.php",
       data: aprform.serialize() + "&action=recheck&viewid="+viewid,
       type: 'POST',
       success: function(data) {
           if (data == 1) {
               swal({
...
$('#hold_Submit').on('click', function(event) {
   ...
   if (hold_days == '') {
      ...
      $.ajax({
          url: "php_function/ajax_bonus_status_updt.php",
          data: aprform.serialize() + "&action=hold&="+viewid,
          type: 'POST',
          success: function(data) {
            // console.log(data);
            // return false;
...
$('#reject_Submit').on('click', function(event) {
   ...
   $.ajax({
       url: "php_function/ajax_bonus_status_updt.php",
       data: aprform.serialize() + "&action=reject&viewid="+viewid,
       type: 'POST',
       success: function(data) {
        //   console.log(data);
        //   return false;
...
$('#release_Submit').on('click', function(event) {
   ...
   $.ajax({
       url: "php_function/ajax_bonus_status_updt.php",
       data: aprform.serialize() + "&action=release&viewid="+viewid,
       type: 'POST',
       success: function(data) {
           if (data == 1) {
               swal({
